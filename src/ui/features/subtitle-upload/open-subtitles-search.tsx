...
export function OpenSubtitlesSearch({
  ...
  const runSearch = async (page: number, append: boolean) => {
    ...
    try {
      const query =
        append && lastSearchQuery.current
      ...
      setResult((current) => {
        ...
        const candidates = [
          ...
          ...response.data.candidates.filter(
            (candidate) =>
              !current.candidates.some((currentCandidate) => currentCandidate.fileId === candidate.fileId)
          ),
        ];
  ...
  const addCandidate = async (candidate: OpenSubtitlesCandidate) => {
    ...
    try {
      ...
      const fileName = downloaded.fileName.trim() || candidate.fileName;
      const subtitle = await registerSubtitleText({
        fileName,
        title: subtitleTitleFromFileName(fileName),
        language: candidate.language,
  ...
  return (
    ...
    >
      ...
      >
          ...
          <div className='mt-2 flex flex-col gap-2'>
            <label className='flex min-w-0 flex-col gap-1 text-[12px] font-medium'>
              ...
              <Select
                ...
                onValueChange={(value) =>
                  updateField('contentType', value as OpenSubtitlesContentTypeFilter)
                }
              >
        ...
        <div className='flex flex-col gap-1 text-wrap text-[11px] text-gray-500'>
          <p>{t('opensubtitles_search_privacy')}</p>
          <p>{t('opensubtitles_search_permission')}</p>
        </div>
        ...
        <div className='flex min-w-0 flex-col gap-2'>
          ...
          {result.page < result.totalPages && (
            <Button
              ...
              type='button'
              disabled={busy}
              onClick={() => void runSearch(result.page + 1, true)}
            >
