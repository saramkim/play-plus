      ...
      <div className='p-2 overflow-y-auto'>
        <ul className='flex flex-col gap-1'>
          {subtitles.map((subtitle, index) => (
            <SubtitleItem key={index} text={subtitle.text} isActive={index === activeIndex} />
          ))}
        </ul>
      </div>
    </div>
...
const SubtitleItem = memo(({ text, isActive }: { text: string; isActive: boolean }) => (
  <li className={cn('p-2 rounded', isActive ? 'bg-primary/20' : 'bg-gray-50')}>
    <p
