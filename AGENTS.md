...
사전 논의 여부는 먼저 위 예외 조건으로 판단한다. 승인된 기대 동작을 복원하는 명확한 버그 수정과 이미 결정된 방향의 구현은 사전 논의를 생략한다. 새로운 제품·데이터·권한·아키텍처 결정이나 실질적인 트레이드오프가 필요한 경우에는 필수 논의 조건을 적용한다. 사전 논의 생략은 사후 검토 생략을 뜻하지 않으며, 제품 계약 변경의 ChatGPT 검토와 사용자 승인 요건은 유지한다.
...
모든 coding task는 다음 순서를 따른다. 이 순서는 새로운 Git·외부 쓰기 권한을 부여하지 않는다. 승인된 범위의 구현·검증·검토자료 준비를 마치고, 요청된 산출물의 완료와 별도 승인 대기 또는 필수 검증 미완료를 구분해 보고한다. 요약만으로 요청된 후속 단계를 대신하지 않는다.
