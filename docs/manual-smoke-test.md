...
The Chrome DevTools continuation below supersedes the pending representative source/revision, panel-close lease and episode-fence cases. Remaining actual cases are native Korean IME composition-commit Enter; same-frozen-context active-practice advertisement suspension/explicit resume; broader source, lease and fence variants; save-in-flight exit and injected restoration/storage failure; populated previous-record reset; and direct-message payload privacy capture. Pre-entry advertisement gating and a route-changing pre-roll that invalidated old practice were observed, but neither certifies same-context advertisement resume. Unicode fill and deterministic IME tests are not native-composition proof. Learner enjoyment/effectiveness observation remains a separate follow-up.
