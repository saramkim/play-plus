...
Remaining actual cases are `NOT RUN`: Korean IME composition-commit Enter; same-frozen-context active-practice advertisement suspension/explicit resume; deliberate learning/support source or revision changes; all episode-fence and lease-timeout variants; save-in-flight exit and injected restoration/storage failure; populated previous-record reset; and direct-message payload privacy capture. Pre-entry advertisement gating and a route-changing pre-roll that invalidated old practice were observed, but neither certifies same-context advertisement resume. The tool cannot emit the Hangul toggle key; a one-time physical input-mode switch was requested but not received during this run. Unicode fill and deterministic IME tests are not native-composition proof. Learner enjoyment/effectiveness observation remains a separate follow-up.
...
ChatGPT's completion review accepted the bounded actual evidence and retained code approval, but withheld Issue #102 completion. The remaining representative core actual gates are native Korean IME, same-context active-practice ad resume, same-document source/revision invalidation, heartbeat-loss/lease recovery, and episode-fence consumption. The fixture supplement closes only the geometry evidence gap. The later browser continuation was stopped by physical Escape; its temporary server and restarted KR gateway were shut down. PR #103 remains Draft.
