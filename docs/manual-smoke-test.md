...
## One-line listening revision (#102, updated 2026-09-08)
...
The following bounded actual observations use production-code head `14c31c8`, reloaded into the existing unpacked extension after unlocking Windows. A row certifies only the observation stated; automated coverage and the unobserved cases below remain separate.

| Actual Chrome observation | Result | Evidence / limits |
| --- | --- | --- |
| Supported player and real side panel | PASS | Dedicated KR preflight passed with unchanged host routing; authenticated English episode, native English learning/Korean support, ready DRM player and no media error. Extension action opened a verified `SIDE_PANEL`. No audio-language detection or human listening claim. |
| Recent completed-line entry | PASS | Repeated entries selected a whole already-ended line within the 10-second bound, played once and stopped. This sample does not independently certify every overlap/grouping edge case. |
| Frozen past-line picker | PASS | Manual entry listed past lines only; reopening the picker after practice retained the original latest eligible boundary. Selection stayed explicit, with no automatic next line. |
| Reveal and hidden replay | PASS | Immediate answer and accepted support appeared. Hide-and-replay removed answer, support, textarea and comparison scaffold before playback; uninterrupted hidden replay produced only the factual completion message. Normal and 0.75x replay were observed. |
| Optional typing and ordinary keyboard | PASS | Shift+Enter inserted a newline; Enter produced non-exact comparison and matched parts without changing the draft or losing textarea focus. Same-line hide/reopen retained the draft and cleared comparison; selecting another line cleared the draft and focused the new textarea. Actual Korean composition is a separate unobserved case below. |
| Attainable narrow layouts | PASS | Actual 390px and 360px side panel in light/dark themes had no visible horizontal overflow and one scroll owner, including comparison. The clipped 1px screen-reader-only span is intentionally hidden. Native browser-chrome resizing clamped at 360px; 320px was unattainable in this window. Theme restored to System. |
| Return and playback ownership | PASS | Return restored the first captured position, rate and paused state, including a return before a selected line finished and after slow replay. Native player play/forward-seek took priority: the old practice closed and playback continued at the user's new position without restoring the older entry. |
| Route invalidation and retry recovery | PASS | Navigating from an active practice to the actual home route immediately removed old practice text. The first end attempt showed an honest retry error while the new document loaded; Retry ending session succeeded and unlocked all four destinations. The navigation tool timed out after 10 seconds, but live readback confirmed the new route and recovery. |
| Explicit save and previous records | PASS | Explicit current-line save added one card and left listeningProgress unchanged. The collapsed previous-records section opened with its original zero-progress meaning and reset controls; populated-record reset was not exercised. Both task-created test cards were deleted through Library; final card count is zero, matching the original pre-test library. |
| Bounded privacy and diagnostics | PASS | Final local/sync/session checks found no transient draft/scaffold/composition/submitted-answer properties or task draft markers. listeningProgress remained unchanged. Panel network contained only extension-owned asset requests. Panel and service-worker error/warn counts were zero; the captured player console had no errors and only platform preload, DRM robustness and telemetry/timer warnings. Direct-message payload capture was not performed. |
| Four destinations after teardown | PASS | Learning, Subtitles, Library and Review remained navigable after teardown; Library and Review correctly showed the empty state after test-card cleanup. |

The continuations below supersede the pending representative source/revision, panel-close lease, episode-fence and active-practice advertisement cases. Remaining actual cases are native Korean IME composition-commit Enter; broader source, lease and fence variants; save-in-flight exit and injected restoration/storage failure; populated previous-record reset; and direct-message payload privacy capture. Pre-entry advertisement gating and a route-changing pre-roll that invalidated old practice were observed, but neither certifies same-context advertisement resume. Unicode fill and deterministic IME tests are not native-composition proof. Learner enjoyment/effectiveness observation remains a separate follow-up.

Automated checks on production-code head `14c31c86f30b9a81f403e53e19f53c9775013ad1` (2026-09-08): `yarn type-check`, `yarn lint --ignore-pattern '.worktrees/**'`, `yarn test:run --exclude '**/.worktrees/**'` (103 files, 1,219 tests), `yarn build` and `git diff --check` passed. Only unrelated nested worktrees were excluded. Build emitted three existing webpack size/performance warnings. [Exact-head CI verify](https://github.com/saramkim/play-plus/actions/runs/34160929421) also passed. ChatGPT reviewed the full PR and all subsequent fixes and approved this exact code with no further code changes required; final Chrome evidence remains a separate gate. These checks do not substitute for the actual Chrome rows above.

The initial profile-ownership blocker was resolved with the user's approval. The dedicated KR gateway passed preflight with unchanged host routing; the signed-in supported English episode had native English learning/Korean support subtitles, a ready DRM player and no media error. The production unpacked extension was opened through its action and verified as a real `SIDE_PANEL`, not an ordinary extension tab.

Bounded observations on `98db304` passed: a recent completed line; immediate reveal of answer and accepted support; optional non-exact comparison with unchanged draft and matched-parts scaffold; hidden replay removing answer/support/typing/scaffold with factual completion; return to the exact captured paused position and rate; explicit current-line save (zero to one test card) with unchanged listeningProgress; and a newer player seek taking priority and closing the old practice. A real 360px internal text overflow was found and corrected with wrapping; the corrected build had zero overflowing visible main elements and one scroll owner. These are historical subset results, not proof of later code or advertisement fixes.

Historical `f89811a` attempt: the real side panel and pre-entry advertisement gate were observed, but severely delayed input ended in the safe stale-session view and the attempted loop remained `UNKNOWN`; the cause was not isolated. Locked Windows prevented native keyboard/IME and browser-chrome resizing. The later `14c31c8` observations above supersede that attempt only for the cases explicitly rerun. Automated response/broadcast-order and stale-request integration tests remain deterministic regression evidence, not actual advertisement certification. Test-card cleanup is now complete and no previous progress was reset.

The dedicated KR gateway was stopped after the 2026-09-08 run; the official stop command returned `ok: true`, `active: false`. No active practice or test card remains. Raw subtitle text, full watched URLs, account details and machine-specific extension identifiers are omitted from this record.

### Geometry fixture and deterministic evidence supplement

The development-only [fixture server](../scripts/listening-geometry.mjs) bundles the current [ListeningMission component fixture](../src/ui/features/listening-mission/listening-mission.geometry.tsx), production stylesheet and existing fonts into a temporary directory. It does not overwrite `dist/`, use account data or exercise extension playback. Run `yarn node scripts/listening-geometry.mjs`, open the printed localhost URL in a foreground browser with at least 390x650 CSS px available, and await `window.geometryReady` in its console. URL parameters select `width=320|360|390`, `sample=english|korean|unbroken`, `locale=en|ko` and `theme=light|dark`. Stop the server with Ctrl+C afterward.

On 2026-09-08, the root agent ran all 36 combinations serially in visible same-origin frames. All 108 measured states (hidden, revealed with typing/comparison, past picker) passed: no horizontal overflow, overlapping controls, clipped 3px focus clearance or unreachable action, and exactly one scroll owner. The browser also confirmed all three Pretendard font weights loaded in every case. These are deterministic geometry observations with synthetic text and controller responses, not actual Side Panel, native IME, playback or screen-reader certification. An earlier server attempt with failed font requests was excluded. A subsequent optional screenshot attempt did not complete; it is not visual evidence.

Before committing this supplement, type-check, canonical-root lint, all 103 test files / 1,219 tests, production build and diff check passed again. Only unrelated `.worktrees/**` were excluded from lint/test discovery; the build retained its three existing size/performance warnings. SHA-256 comparison confirmed `dist/index.js`, `dist/content.js` and `dist/background.js` were byte-identical before and after adding the fixture. The fixture does not enter the shipped extension bundles.

The following test/claim mapping uses tests already passing on production-code head `14c31c8`. It explains deterministic coverage without changing any unperformed manual row to PASS.

| Claim | Exact test evidence | Boundaries proved |
| --- | --- | --- |
| Exit during pending save | `listening-mission.test.tsx`: `only saves the explicit current selection and keeps exit available during a save`; `listening-session-coordinator.test.ts`: `does not let a late explicit save rewind or write after the session has ended` | Only the selected key is requested, Return remains available, and a stale late save neither writes a card nor rewinds playback. |
| Restoration error and newer user intent | `listening-mission.test.tsx`: `retries failed restoration directly with no discard/progress dialog`; coordinator: `preserves a user seek during a delayed restoration failure and its retry` | Error stays retryable; retry succeeds without a progress/discard gate or overwriting the newer seek. |
| Save error and partial result | coordinator: `returns learning-only and transport error without leaking or terminating the session`; adapter: `maps playback, saves selected difficult keys in order, and preserves terminal truth` | Learning-only success remains distinguishable, errors are sanitized, the session survives retryable failure, and selected save results retain their status. |
| Previous-record reset success, cancellation and failure | `listening-flow.test.tsx`: `uses separate reset confirmations, preserves UI on failure, and restores focus`, `cancels an open video reset when its exact tab/source context changes`; `listening-progress-storage.test.ts`: `allows clear-video and clear-all to retry after their writes fail`, `clears only the exact video namespace and then clears all progress` | Escape/context cancellation does not issue a reset; failed writes preserve populated records; retry works; video reset preserves another video's namespace. Populated reset was not exercised in actual Chrome. |
| No new practice progress capability | adapter: `has no progress-writing capability during active practice`; mission: `allows exit during playback without saving progress and ignores the late replay response` | Current practice has no progress commit capability and cannot turn a late replay response into a hidden-playback completion after exit. |
| Direct transport and background boundary | adapter: `uses only direct guarded catalog/begin messages and runtime progress facts`; `message-handler.test.ts` in background: `rejects malformed or extra listening progress params before touching storage` | Catalog/session snapshots stay on the direct UI-content path; background progress envelopes reject extra answer text before storage. Explicit canonical card saving remains an allowed exception. |
| Resume and lease races | adapter: `pauses old-binding heartbeats across a delayed resume and ignores their late terminal responses`, `renews the lease beyond 15 seconds while a retryable end error is visible`; `listening-flow-session.test.tsx`: `preserves the ad draft with %s without early interaction, autoplay or completion` (response-first/broadcast-first) and `allows return while a confirmed resume awaits its broadcast` | Old responses cannot terminate the rebound owner, retryable end errors retain lease renewal, duplicate resume is blocked and Return remains available. These are not natural-ad or actual panel-close proof. |

Paths above are under `src/ui/features/listening-mission/`, `src/ui/features/listening-flow/`, `src/ui/adapters/`, `src/content/features/listening-session/`, `src/storage/v2/` and `src/background/` respectively. The production `dist/background.js` audit found zero occurrences of `answerText`, `inputDraft`, `scaffold`, `missionSnapshot`, `catalogBody` and `rawCue`; manifest, package, production webpack configuration and v2 schema have no diff from the PR base. This static/transport evidence complements the bounded actual Storage/network/log checks; it does not claim a complete capture of every runtime message or transient write.

ChatGPT's completion review before the continuation below accepted the bounded actual evidence and retained code approval, but withheld Issue #102 completion. At that point the five remaining representative core actual gates were native Korean IME, same-context active-practice ad resume, same-document source/revision invalidation, heartbeat-loss/lease recovery, and episode-fence consumption. The fixture supplement closed only the geometry evidence gap. That browser continuation was stopped by physical Escape; its temporary server and restarted KR gateway were shut down.

### Chrome DevTools continuation on `6131881` (2026-09-08)

The user explicitly requested Chrome DevTools MCP verification. The dedicated KR gateway passed preflight with unchanged host routing; the existing unpacked extension was identified and reloaded from stable `dist/`. This candidate contains the byte-identical production bundles described above. [CI on `6131881`](https://github.com/saramkim/play-plus/actions/runs/34166448117) passed. All interactions and observations in this continuation used `chrome-devtools-kr`; Windows Computer Use was not used.

| Representative actual gate | Result | Evidence / limits |
| --- | --- | --- |
| Same-document source/revision invalidation | PASS | In the actual Side Panel, reveal showed both answer and support and an optional draft was entered. A temporary ordinary extension tab was used solely to operate the existing Learning languages form while practice locked its own settings. Saving support language from Korean to none changed native support identity and subtitle revision while content instance, document/route, video and attachment stayed unchanged. The actual Side Panel removed answer and draft, showed safe termination and unlocked all four navigation destinations. The original English/Korean profile was restored through the form and verified; progress remained unchanged. The auxiliary tab was closed. No ordinary-tab observation is claimed as Side Panel proof. |
| Actual panel close and heartbeat lease | PASS | After capturing a paused entry position at normal rate, a real Side Panel practice moved playback to its selected line and suppressed the on-video controller and subtitles. MCP closed the actual Side Panel target without issuing Return/end. Read-only media event observation showed seeking back to the entry 14,980 ms after the close request and seek completion at 15,047 ms. Position, rate and paused state were restored; the controller and subtitles reappeared. Reopening via the extension action yielded a fresh actual Side Panel with start controls and reset confirmation. This covers the representative heartbeat-loss recovery, not every lease/error permutation. |
| Valid episode fence and last eligible whole line | PASS | A normal page reload reacquired current provider playback evidence after the source-change test. The actual response had one strict six-field watch-next marker, valid ordered intro markers, raw-duration scaling and current-media bounds. The native overview contained cues beyond the fence, while every listening catalog segment ended within it. DevTools set a paused test entry just after the final catalog segment and before the fence; the actual Side Panel's recent-line action selected that final whole line. Media seek/play/pause/timeupdate observations confirmed normal and 0.75x hidden replay started at the final line, stopped at its end and stayed within the fence; the UI showed only factual hidden-listen completion and no automatic next line. Return restored the captured entry position, rate and paused state. This is the representative valid-fence path, not every invalid-marker or drift variant. |
| Same-context active-practice advertisement resume | NOT RUN | Real pre-entry advertisements were observed, including after the reload above, but no advertisement interrupted an active practice in this bounded run. No ad flags, provider response or lifecycle event was fabricated to claim natural-ad proof. |
| Native Korean IME composition-commit Enter | NOT RUN | Available DevTools keyboard/text tools did not provide a verified native Windows Korean composition sequence. No Unicode fill or synthetic composition event was counted as this proof. Ordinary Enter/Shift+Enter and deterministic IME tests remain separate evidence. |

The reload navigation call timed out after 10 seconds; live page readback subsequently confirmed successful navigation. A pre-entry advertisement temporarily stalled with buffered media and no player error; bringing the actual player page to the foreground through DevTools restored playback and content readiness. The gateway status check confirmed active KR egress and isolated host routing. These environment observations did not require a product change. Final Side Panel warning/error count was zero; the original English/Korean learning profile, empty progress and zero-card library were verified after teardown. No test card was created in this continuation.

The player was returned to the paused position used before the fence test. The official gateway stop command returned `ok: true`, `active: false`. No active practice remains. Three representative core gates are now closed; native Korean IME and same-context active-practice ad resume remain pending. PR #103 remains Draft.

### Advertisement replay correction (2026-09-08)

This continuation supersedes the pending advertisement row above. The original short episode's actual playback response scheduled only pre-roll ads. A supported native English/Korean movie supplied real mid-roll slots, allowing the active-practice boundary to be exercised without altering provider responses, ad flags, lifecycle events or extension messages.

The pre-fix candidate `ea70343` failed the post-ad replay boundary twice. After explicit resume, Coupang's main video was already playing. Seeking to the practice start completed with the media clock approximately 16 ms ahead of the target. The coordinator required a seeked position within 1 ms before installing playback-end observers, so it missed that event: the ready player continued beyond the line and the UI remained in Listening. Return still restored the captured entry. The first attempt did not establish uninterrupted completion; the second confirmed forward time updates beyond the line, distinguishing the defect from buffering.

The correction pauses the owned video immediately before each clip seek. It preserves the existing identity/generation/seek checks and does not add playback to the explicit ad-resume action. Two coordinator regressions simulate a replacement video already playing and advancing 16 ms before seeked; both 1x and 0.75x cases failed before the fix and passed afterward. The complete coordinator suite (55 tests), type-check, canonical-root lint, all 103 files / 1,221 tests, production build and diff check passed. Only unrelated `.worktrees/**` were excluded; the build retained three existing size/performance warnings.

The corrected production build was reloaded into the discovered unpacked extension, and the actual movie document was reloaded to acquire the new content script. Its `dist/content.js` SHA-256 was `AA314237D8A2EA759EC1A34FB1972746673F22B95563AA99C84105F3FD1DF010`; UI/background bundles were unchanged. The following observations use that correction, not the pre-fix candidate.

| Corrected-build actual observation | Result | Evidence / limits |
| --- | --- | --- |
| Active practice and real mid-roll suspension | PASS | DevTools prepared a paused entry just after an eligible whole line crossing a real scheduled ad slot, then the actual Side Panel start action selected the line. Because the ad followed quickly, a DOM observer operated the existing optional-typing control and dispatched an input event to enter a synthetic draft before suspension. This is programmatic UI setup, not native IME proof. A real mid-roll VAST request returned 200 and a short advertisement video played. The extension suspended practice and required explicit resume while instance, epoch, content and subtitle identities remained unchanged and the attachment changed. |
| Explicit resume, same selection and draft | PASS | After the real ad ended, the Side Panel required Return to practice. That action produced no new play, pause, seek or rate event; the platform's already-playing main video was distinct from a practice replay. The optional typing control reopened with the same draft, and no interrupted-playback completion was displayed. |
| Rebound-media replay and completion | PASS | Explicit hidden 1x replay paused before seek completion, played on the new main-content attachment, stopped at the selected line endpoint and displayed factual hidden-listen completion. Explicit 0.75x replay also stopped at that endpoint and displayed completion. This reruns the exact post-ad path that failed before the fix. |
| Return and cleanup | PASS | Return restored the captured entry position, normal rate and paused state, removed the practice and unlocked navigation. Profile remained English/Korean; progress stayed empty and the library stayed at zero cards. Task draft markers were absent from local/sync/session Storage and Side Panel warning/error count was zero. The temporary movie tab and response-inspection file were removed, the movie position was restored, and the official gateway stop returned `ok: true`, `active: false`. |
| Windows native Korean IME | NOT RUN | A native key sent to the actual textarea produced trusted ordinary key/input events but no Korean composition. The Windows input tool rejected `Hangul` as unsupported. A one-time physical switch to Korean input was requested and remained unanswered. Synthetic draft input used for the ad test is not native-composition evidence. |

ChatGPT independently reviewed correction `909d79a0e8e7f4f0e57478069a0aaa70027dabf4`, its surrounding ownership/seek/completion implementation, the complete increment and exact-head CI, and approved it with no additional required code fixes. The submitted corrected-build advertisement evidence was accepted and that representative gate is closed. This is source review and acceptance of the execution record, not an independent browser replay by the reviewer. Earlier accepted observations retain their original head attribution.
...
| Full subtitles transient scrollbar owner | NOT RUN | actual Extension Pages target의 Full subtitles 화면에서 materialize된 seek button 25개를 DevTools-emulated 360/390px viewport로 구조 측정. programmatic focus에서는 tooltip이 열리지 않았고, DOM-dispatched pointer event로 열린 tooltip 전후 document/body/main/list owner 치수·viewport inset은 불변, tooltip은 227×67이며 내부 overflow가 없었음. 이는 실제 hardware pointer/CSS `:hover`와 transient scrollbar paint를 실행한 증거가 아니므로 실제 mouse-hover 현상과 scroll owner 판정은 `NOT RUN` |
...
| --- | --- | --- | --- |
| Review-head identity | PR head SHA, #89 squash merge ancestry, clean scoped diff and exact-head CI | PASS | #89 squash merge `f56ad3fd8b83411d79e0b862f2ee0681b1ddefd2`에서 분기한 PR #94이며 runtime correction head `11b5febae55703bb25785eeb2337507f26f492b8`와 final documentation head를 각각 build/reload; 최종 PR head SHA와 terminal CI는 PR body에 기록 |
...
| GitHub exact-head checks | PR required checks terminal success on the same head | NOT RUN | final documentation head push 뒤 terminal result를 PR body에 기록하며 이전 SHA 결과를 승계하지 않음 |
...
| Exact classifier and exclusions | `src/content/coupang-play.test.ts`, `src/content/native-subtitle-variant-qualification.test.ts`: exact case-sensitive `L` / `L sdh`, one ASCII space, non-subtitle/case/region/CC/captions/malformed sibling exclusion | PASS | exact fixtures에서 trim, alias, label, MIME, default, metadata 또는 cue-content inference가 없음을 확인 |
| Candidate-local settlement | 같은 focused suites: fetch rejection, body-read rejection, VTT parse rejection, empty parse와 usable sibling retention | PASS | strict top-level playback envelope failure는 acquisition-wide이고 candidate-local failure만 격리됨 |
| URL and deterministic selection | 같은 focused suites: direct `src`, nullish `sources[0].src`, both-present precedence, no usable URL, regular only, SDH only, regular+SDH, duplicate usable regular/SDH와 usable-count selection | PASS | second URL, retry, new playback/discover lookup, provider fallback 또는 prefetch 없음 |
| Complete atomic snapshot | `src/content/features/subtitle/subtitle-store.test.ts`, `src/content/message-handler.test.ts`: complete-result replacement, physical category/identity change, cue change, identical snapshot no-op and exactly-one `subtitleRevision` transition | PASS | complete snapshot만 한 번 적용되고 identical snapshot은 no-op |
| Fresh P0 acceptance | message-handler, playback-context and video-lifecycle focused suites: route/video/content instance, `contentEpoch`, attachment, lifecycle, source and request drift rejection | PASS | advertisement/waiting/placeholder/transitioning, unsupported route, SPA/content/attachment/source drift를 fail closed 처리 |
| Subtitle-bound invalidation | transcript/overview, seek/repeat/clip/save, catalog and Listening Mission coordinator/UI focused suites | PASS | changed snapshot은 모든 subtitle-bound work를 함께 무효화하고 identical snapshot은 catalog/Mission churn을 만들지 않음 |
| Existing behavior regression | Coupang playback, native/registered role, source identity, subtitle overview/save/seek/repeat and Listening Mission focused suites | PASS | logical source, Library display와 Listening Progress가 계속 `native:<language>`임을 포함 |
| Full local gates | `yarn type-check`, `yarn lint`, `yarn test:run`, `yarn build`, `git diff --check` | PASS | 102 files / 1,163 tests, production build 성공; 기존 bundle-size warning 3건만 존재 |
| Static privacy/config boundary | source와 built output의 message, Storage/schema/migration, permission/host/CSP/dependency, network/replay/discover/retry/prefetch/interception/logging audit | PASS | 새 product message surface, key/write, permission prompt/grant, request path 또는 sensitive persistence/relay/logging 없음 |
| GitHub exact-head checks | PR의 required checks가 같은 review head SHA에서 terminal success | NOT RUN | 이전 SHA의 checks는 승계하지 않음 |

### Actual Korean Chrome required core gate

Rebuilt production `dist/`, dedicated Korean egress, signed-in supported Coupang Play movie/episode tab과 action-opened tab-bound Chrome Extension Pages Side Panel을 사용한다. 설치, 인증, 지역, DRM/player와 native acquisition은 서로 독립된 gate다. Mandatory actual-Chrome 행은 모두 실제 `PASS`여야 한다. `Evidence-equivalent core` 행은 manual PASS를 자동화로 대체하는 행이 아니라 canonical §9.4가 정의한 composite verification gate다. 표의 `PASS`는 해당 행의 exact-head deterministic/static evidence와 required actual representative evidence가 모두 PASS라는 뜻이다. 자연 발생하지 않은 trigger 자체는 별도로 `NOT OBSERVED WITHIN SAMPLE`로 기록하며 manual observation으로 승격하지 않는다.

Canonical §9.4가 허용한 artifact-identity carry-forward row의 `PASS`는 final review head에서 category를 다시 관찰했다는 뜻이 아니다. 해당 row는 originating observation head, final review head, byte-identical production-artifact proof와 `not re-observed on this head` provenance를 반드시 함께 기록한다.

| Check | Gate | Result | Required observation / evidence |
| --- | --- | --- | --- |
| Exact-head production extension | Core | PASS | clean committed review head에서 production `dist/`를 다시 build/reload하고 action이 연 실제 Extension Pages Side Panel에서 supported route, logical native, non-empty Overview와 Overview/Catalog/content revision 일치를 재확인; final SHA는 PR body에 기록 |
...
| Regular canonical positive | Core — artifact-identity carry-forward | PASS | Category attribution was directly observed in actual Korean Chrome on certified runtime head `4e1aa9503e092a84a71a62831efa1990d27b858a`. The final review head is a documentation-only descendant and rebuilt production `dist` has an identical complete file-path + byte-digest set. Final-head KR/auth/DRM/player, non-empty logical-native projection and required consumer/boundary revalidation PASS. Category attribution carried forward by byte-identical runtime artifact; not re-observed on this head. |
| Regular-absent exact SDH positive | Core — artifact-identity carry-forward | PASS | Regular-absent plus exact SDH category attribution was directly observed in actual Korean Chrome on certified runtime head `4e1aa9503e092a84a71a62831efa1990d27b858a`. The final review head is a documentation-only descendant and rebuilt production `dist` has an identical complete file-path + byte-digest set. Final-head KR/auth/DRM/player, non-empty logical-native projection and required consumer/boundary revalidation PASS. Category attribution carried forward by byte-identical runtime artifact; not re-observed on this head. |
| Complete P0 stale acceptance matrix | Evidence-equivalent core | PASS | Exact-head focused tests가 route/video/content instance, `contentEpoch`, attachment, lifecycle, source와 request/late-response drift rejection 전부를 PASS했다. Actual Chrome에서는 advertisement/content 및 실제 route transition에서 old Mission 제거, stale heartbeat와 current route/video identity 밖 ready snapshot 비수용을 확인해 representative P0 invalidation을 PASS했다. Slow 3G bounded return 안에서 모든 late-response/attachment/source axis가 각각 자연 완결되지는 않았지만 그 개별 deterministic axes는 exact-head matrix로 증명하며 manual PASS로 주장하지 않는다. |
...
| Identical accepted snapshot no-churn | Evidence-equivalent core | PASS | Exact-head subtitle-store/message-handler/coordinator tests가 identical physical identity + category + cue snapshot을 no-op으로 처리해 cache, revision, catalog와 Mission churn 0을 증명했다. Actual Chrome bounded steady-state에서는 revision/source가 안정적이고 unexpected Mission churn이 없었다. 광고 또는 same-content replay trigger는 이번 bounded sample에서 `NOT OBSERVED WITHIN SAMPLE`; 재전달 자체를 manual PASS로 주장하지 않는다. |
| Existing replay and one-attempt/no-retry boundary | Evidence-equivalent core | PASS | Exact-head fetch-call ledger가 selected candidate마다 approved `src ?? sources[0].src` 한 URL 한 attempt, second URL/retry 0을 증명했고 source/built audit에서 playback/discover/provider retry, prefetch, interception path가 없음을 확인했다. Actual Chrome aggregate ledger는 page playback 1회, existing extension replay 1회, successful candidate GETs, failed/blocked 0과 external subtitle lookup 0을 확인했다. Chrome DevTools의 page-player/content-script VTT initiator 분리는 acceptance requirement가 아니며 raw response/file은 기록하지 않았다. |
...
#89 verification status: **PASS SUBJECT TO FINAL EXACT-HEAD REVIEW**. The regular canonical and regular-absent exact SDH category attributions were directly observed on certified runtime head `4e1aa9503e092a84a71a62831efa1990d27b858a` and are carried forward only through the canonical byte-identical artifact rule; they are not claimed as newly re-observed on the final documentation-only head. All other mandatory actual Chrome gates are revalidated on the final review head. The three evidence-equivalence axes separately satisfy their canonical composite gates. Any artifact-identity mismatch, runtime/build/manifest/dependency change, exact-head deterministic/static/CI failure or mandatory actual regression reopens the gate. Release, tag, deployment, Store submission, merge or direct `main` push is not authorized by this evidence record alone.
