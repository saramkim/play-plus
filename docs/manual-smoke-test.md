...
| OpenSubtitles Consumer / plan | UNKNOWN — approved one-time local credential로 direct production search/download는 성공했지만 Provider Consumer approval, plan/quota와 attribution 조건은 독립 확인하지 못함; release blocker 유지 |
| OpenSubtitles build environment | PASS (technical subset) — 사용자 승인된 기존 local credential을 검증 build process에만 주입해 keyed `dist`를 실행하고 값 비노출 header boolean으로 확인; 검증 뒤 keyless canonical build를 복원하고 세 KR profile을 reload. Provider 승인 여부는 별도 `UNKNOWN` |
...
| `yarn test:run` | PASS | production-code verification head `0252e67`에서 `yarn.cmd test:run`: 97 files, 895 tests |
...
| GitHub CI on recorded evidence head | PASS | current pre-documentation PR head `b235d1e0435d176f8bad0bf18322a37e6346ae0c`의 required checks terminal success 확인; final evidence-only documentation head의 exact-head CI는 PR #72 checks/body에 별도로 기록 |
...
| --- | --- | --- |
| Full local gates | PASS | type-check, lint, 97 files / 895 tests, production build, diff-check |
...
실제 signed-in player, registered SRT/VTT/SMI 관리, native/registered catalog와 bounded Mission 흐름, platform-caption on/off ownership, dedicated clean fresh install/readiness, dedicated signed-in actual-v1.11 same-path upgrade, OpenSubtitles permission/direct search/selected download/cache subset, real Windows Korean IME, strict progress facts와 bounded Library/Review 회귀는 이번 exact candidate에서 직접 확인했다. 그러나 worker-only restart, actual ~390px, Provider Consumer approval/plan/quota/attribution `UNKNOWN`과 나머지 compound row가 `NOT RUN` 또는 `UNKNOWN`이므로 #66의 현재 증거 결론은 계속 **INSUFFICIENT EVIDENCE**다. 이는 acceptance waiver가 아니며 merge 또는 다음 릴리스 판단으로 해석하지 않는다.
...
| Provider production gate | Approved Consumer and documented plan | Record login/JWT-free behavior, direct exact origins, quota/plan and attribution terms without recording credentials; mark `UNKNOWN` and block release if OpenSubtitles approval or any contract term cannot be confirmed | UNKNOWN | Login/JWT-free direct exact-origin search/download, technical quota-bearing success copy와 keyless fail-closed는 확인; Provider Consumer approval, plan/quota와 attribution 조건은 독립 확인하지 못해 release blocker 유지 |
...
Issue #66 certification status on 2026-08-13: **INSUFFICIENT EVIDENCE**. Batch #67 production-code verification head `0252e67`의 exact candidate는 automated/static gates, dedicated clean fresh install/readiness, dedicated signed-in actual-v1.11 same-path upgrade와 rollback, bounded actual signed-in `/en/play/<video-id>/episode`, DRM/player, native/registered SRT/VTT/SMI subtitle, OpenSubtitles permission/direct search/selected download/cache subset, platform-caption on/off ownership, real Windows Korean IME와 360px Mission/Library/Review subsets을 통과했다. 그러나 worker-only restart variants, approximately 390px의 exact protocol error, Provider Consumer approval/plan/quota/attribution `UNKNOWN`과 나머지 compound rows는 계속 `NOT RUN` 또는 `UNKNOWN`이며 waiver가 아니다. 상세 matrix는 `PASS 44 / NOT RUN 148 / FAIL 0 / UNKNOWN 1`이다. Release, tag, deployment, Store submission, merge와 direct `main` push는 수행하지 않았다.
