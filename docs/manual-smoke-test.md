...
| Review session | N/A | Enter Review and verify the fixed New/Learning snapshot keeps its order and total; confirm the exact Learning/New counts and current status badge; reveal the optional second subtitle; use Previous/Skip without changing status; verify New, Learning, and Mastered re-review each show their own explicit status actions; persist the chosen status before automatic advance; revisit a judged card and confirm its latest status/actions; finish the queue and re-enter with completed cards only | NOT RUN | |
| Review library | Both | Switch between Review and Library; search either subtitle line, sort by saved date, filter All/New/Learning/Mastered, change status, and delete/undo without losing the focused Review entry point | NOT RUN | |
| Review accessibility | N/A | At 320/360/390px in light and dark mode, verify no horizontal overflow or clipped long text; use every mode, reveal, navigation, judgment, filter, sort, video, copy, delete, and clear action by keyboard; verify progress and `aria-expanded` state are announced accurately | NOT RUN | |
| Review persistence | Both | Default migrated/new cards to New; preserve status through reopen, delete/undo, source/copy, and backup/restore; verify a failed status write does not advance the focused card | NOT RUN | |
