...
Automated checks on production-code head `f89811ab9ef70d5f433541c86ae694c3f2f4c9e9` (2026-09-07): `yarn type-check`, `yarn lint --ignore-pattern '.worktrees/**'`, `yarn test:run --exclude '**/.worktrees/**'` (103 files, 1,215 tests), `yarn build` and `git diff --check` passed. Only unrelated nested worktrees were excluded. Build emitted three existing webpack size/performance warnings. [Exact-head CI verify](https://github.com/saramkim/play-plus/actions/runs/34116594170) also passed. These checks do not substitute for the actual Chrome rows above.
...
Bounded observations on `98db304` passed: a recent completed line; immediate reveal of answer and accepted support; optional non-exact comparison with unchanged draft and matched-parts scaffold; hidden replay removing answer/support/typing/scaffold with factual completion; return to the exact captured paused position and rate; explicit current-line save (zero to one test card) with unchanged listeningProgress; and a newer player seek taking priority and closing the old practice. A real 360px internal text overflow was found and corrected with wrapping; the corrected build had zero overflowing visible main elements and one scroll owner. These are subset results on that code revision, not completion of the compound rows above or proof of the later advertisement fixes.

The `f89811a` build was reloaded into the same extension and action-opened real 360x778 side panel. Pre-entry advertisement gating, subsequent supported-content readiness and paused player capture were observed. The attempted recent-entry loop is `UNKNOWN`: after severely delayed browser input, the next snapshot was already the safe stale-session exit with no old practice text. Its cause was not isolated; repeat the loop with responsive browser control before claiming success or attributing a product defect. Full latest-code interaction, active-practice advertisement/resume, actual Korean IME, 390px layout, and the remaining ownership/privacy matrix are not yet certified. Windows was locked during the resumed validation, preventing native keyboard/IME and browser-chrome resizing. Keep the compound rows `NOT RUN` until their remaining observations are completed. The automated response/broadcast-order and stale-request integration tests are deterministic regression evidence only. One explicitly saved test card from the earlier run remains in the dedicated profile pending cleanup; the pre-test library was empty and no previous progress was reset.
